...
class GridDisplay():
    ...
    class Grid():
        ...
        def __next__(self):
            self.iter_x += 1
            if self.iter_x >= self.__col_count:
                self.iter_x = 0
                self.iter_y += 1
                if self.iter_y >= self.__row_count:
                    raise StopIteration
            return self.getTile(self.iter_x, self.iter_y)

        def resize(self, ratio):
            self.pos_y = int(self.pos_y * ratio[1])
            self.pos_x = int(self.pos_x * ratio[0])
            self.size = (int(self.size[0] * ratio[0]), int(self.size[1] * ratio[1]))
            self.populate(self.__row_count, self.__col_count)
            self.font = pygame.font.Font(self.fontFace, (int(self.__tile_width * self.fontRatio)))
        ...
        def populate(self, x_count: int, y_count: int):
            if x_count == 0 or y_count == 0: return
            ...
            if len(self.board) == 0 or (len(self.board) != self.__col_count or len(self.board[0]) != self.__row_count):
                for _ in range(self.__row_count):
                    ...
                    for __ in range(self.__col_count):
                        self.board[-1].append(GridDisplay.Tile(self.tileColor, self.textColor))
            ...
            for row in self.board:
                ...
                for tile in row:
                    ...
                    if tile.text is not None:
                        text = self.font.render(tile.text, True, tile.textColor)
                        text_rect = text.get_rect(
                            center=(self.pos_x + self.__offset_x + c * self.__tile_width + self.__tile_width / 2,
                                    self.pos_y + self.__offset_y + r * self.__tile_height + self.__tile_height / 2))
                        if text_rect.width > self.__tile_width:
                            tmp_font = pygame.font.Font(self.fontFace, int(self.__tile_width * 0.8))
                            text = tmp_font.render(tile.text, True, tile.textColor)
                            text_rect = text.get_rect(
                                center=(self.pos_x + self.__offset_x + c * self.__tile_width + self.__tile_width / 2,
                                        self.pos_y + self.__offset_y + r * self.__tile_height + self.__tile_height / 2))
                        if text_rect.height > self.__tile_height:
                            tmp_font = pygame.font.Font(self.fontFace, int(self.__tile_height * 1))
                            text = tmp_font.render(tile.text, True, tile.textColor)
                            text_rect = text.get_rect(
                                center=(self.pos_x + self.__offset_x + c * self.__tile_width + self.__tile_width / 2,
                                        self.pos_y + self.__offset_y + r * self.__tile_height + self.__tile_height / 2))
    ...
    def resizeWindow(self, resizeEvent, scale=True):
        self.size = self.screen.get_size()
        self.screen = pygame.display.set_mode((resizeEvent.w, resizeEvent.h),
                                              pygame.RESIZABLE)
        if scale:
            ratio = (resizeEvent.w / self.size[0], resizeEvent.h / self.size[1])
            self.fontSize = int(self.fontSize * ratio[1])
            self.font = pygame.font.Font(self.fontFace, self.fontSize)
            for grid in self.toRender:
                grid.resize(ratio)
